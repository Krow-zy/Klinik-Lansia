...
<label class="flex flex-col gap-1">
	...
	<input name="tanggal_lahir" type="date" class="border p-2 rounded" value="{{ old('tanggal_lahir', isset($lansia) ? $lansia->tanggal_lahir->format('Y-m-d') : '') }}" required />
</label>
<label class="flex flex-col gap-1">
	<span>Tanggal *</span>
	<input name="tanggal" type="date" class="border p-2 rounded" value="{{ old('tanggal', isset($lansia) ? $lansia->tanggal->format('Y-m-d') : now()->toDateString()) }}" required />
</label>
